...
namespace Eduzo.Games.CookieMonster.Gameplay
{
    ...
    /// </summary>
    public class CookieMonsterDistributionManager : MonoBehaviour
    {
        ...
        public void Initialize(CookieMonsterEventManager cookieMonsterEventManager, CookieMonsterScoreManager scoreManager)
        {
            _cookieMonsterEventManager = cookieMonsterEventManager;
            _scoreManager = scoreManager;

            // Subscribe to events
            _cookieMonsterEventManager.Subscribe<QuestionGeneratedEvent>(OnQuestionGenerated);
            _cookieMonsterEventManager.Subscribe<CookieDroppedOnMonsterEvent>(OnCookieDropped);
        }
        ...
        private void OnDestroy()
        {
            if (_cookieMonsterEventManager != null)
            {
                ...
                _cookieMonsterEventManager.Unsubscribe<CookieDroppedOnMonsterEvent>(OnCookieDropped);
            }
        }
        ...
        private void OnQuestionGenerated(QuestionGeneratedEvent evt)
        {
            ...
            _cookieMonsterEventManager.Publish(new CookiePileUpdatedEvent
            {
        ...
        private void StartDistributionRound()
        {
            ...
            if (_remainingCookies < _divisor)
            {
                ...
                _cookieMonsterEventManager.Publish(new RemainderErrorEvent
                {
                    remainingCookies = _remainingCookies,
                    divisor = _divisor
                });
                ...
                _cookieMonsterEventManager.Publish(new ShowAnswerInputEvent());
                return;
            }
            ...
            _cookieMonsterEventManager.Publish(new DistributionRoundStartedEvent
            {
        ...
        private void DistributeCookiesToAllMonsters()
        {
            ...
            _cookieMonsterEventManager.Publish(new AllMonstersReceivedCookieEvent
            {
                roundNumber = _currentRound
            });
        ...
        private void CompleteDistributionRound()
        {
            ...
            _cookieMonsterEventManager.Publish(new DistributionRoundCompletedEvent
            {
                roundNumber = _currentRound,
                cookiesPerMonster = 1,
                remainingCookies = _remainingCookies,
