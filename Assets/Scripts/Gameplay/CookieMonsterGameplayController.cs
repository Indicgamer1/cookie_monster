...
namespace Eduzo.Games.CookieMonster.Gameplay
{
    ...
    /// </summary>
    public class CookieMonsterGameplayController : MonoBehaviour
    {
        [Header("References")]
        [SerializeField] private CookieMonster_MonsterController[] _allMonsters; // All 5 monsters
        [SerializeField] private CookieMonsterScoreManager _scoreManager;
        ...
        public void Initialize()
        {
            ...
            if (_scoreManager == null)
                _scoreManager = FindObjectOfType<CookieMonsterScoreManager>();

            if (_livesManager == null)
                _livesManager = FindObjectOfType<CookieMonsterLivesManager>();

            if (_allMonsters == null || _allMonsters.Length == 0)
            {
                _allMonsters = FindObjectsOfType<CookieMonster_MonsterController>();
            }

            // Unsubscribe first to avoid duplicate subscriptions on restart
            _cookieMonsterEventManager.Unsubscribe<CookieDroppedOnMonsterEvent>(OnCookieDroppedOnMonster);
            // Subscribe to cookie drop events for validation
            _cookieMonsterEventManager.Subscribe<CookieDroppedOnMonsterEvent>(OnCookieDroppedOnMonster);
            ...
            var gameplayState = stateManager?.GetCurrentState() as CookieMonsterGameplayState;
    
            if (gameplayState != null && gameplayState.IsPracticeMode)
            {
                _totalQuestions = int.MaxValue;
            }
            else if (CookieMonsterNewQuestionGenerator.Instance != null && CookieMonsterNewQuestionGenerator.Instance.GetTotalQuestions() > 0)
            {
                _totalQuestions = CookieMonsterNewQuestionGenerator.Instance.GetTotalQuestions();
        ...
        private (int dividend, int divisor)? GetNextQuestion()
        {
            ...
            {
                return CookieMonsterNewQuestionGenerator.Instance.GetNextQuestion();
            }
        ...
        private void OnCookieDroppedOnMonster(CookieDroppedOnMonsterEvent evt)
        {
            ...
            CookieMonster_MonsterController targetCookieMonsterMonster = null;
            foreach (var monster in _allMonsters)
            {
                if (monster != null && monster.GetMonsterId() == evt.monsterId)
                {
                    targetCookieMonsterMonster = monster;
                    break;
                }
            }
            ...
            int monsterIndex = targetCookieMonsterMonster.GetMonsterId();
        ...
        public void OnGameOver()
        {
            ...
            _cookieMonsterEventManager.Publish(new GameOverEvent
            {
